...
export const CHAIN_ID: SupportedChains = parseInt(process.env.VITE_CHAIN_ID ?? '4');

export const ETHERSCAN_API_KEY = process.env.VITE_ETHERSCAN_API_KEY ?? '';

export const WALLET_CONNECT_V2_PROJECT_ID =
  process.env.VITE_WALLET_CONNECT_V2_PROJECT_ID ?? '';

const INFURA_PROJECT_ID = process.env.VITE_INFURA_PROJECT_ID;

export const createNetworkHttpUrl = (network: string): string => {
  const custom = process.env[`VITE_${network.toUpperCase()}_JSONRPC`];
  return custom || `https://${network}.infura.io/v3/${INFURA_PROJECT_ID}`;
};

export const createNetworkWsUrl = (network: string): string => {
  const custom = process.env[`VITE_${network.toUpperCase()}_WSRPC`];
...
const app: Record<SupportedChains, AppConfig> = {
  [ChainId.Goerli]: {
    ...
    enableHistory: process.env.VITE_ENABLE_HISTORY === 'true',
  },
  [ChainId_Sepolia]: {
